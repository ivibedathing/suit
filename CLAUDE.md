...
- **Viewer & editing**: `FileViewerPane.swift` (+ `Editing`, `Symbols`, `TimeTravel`,
  ...
  `MarkdownRenderer.swift` + `MarkdownHTML.swift` (the Foundation-only whitelist parser for the
  raw-HTML subset READMEs use — harness-covered, and it fails closed by design, so widening it
  is a deliberate decision rather than a drift), `ImagePane.swift`, `PDFPane.swift`.
