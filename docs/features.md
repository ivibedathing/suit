...
- **Per-phase model & effort routing** — a phase's `ROADMAP.md` body can carry `model:` and/or
  ...
  mid-sentence never triggers. The in-repo annotation is the explicit, versioned opt-in. The
  review gate's model is separate (the review-gate model field in Settings ▸ Autopilot,
  empty = default).
