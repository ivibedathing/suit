...
open class TerminalView: NSView, NSTextInputClient, NSUserInterfaceValidations, TerminalDelegate {
    ...
    private func makeMetalView(frame: CGRect, device: MTLDevice) -> MTKView {
        ...
        if let metalLayer = mtkView.layer as? CAMetalLayer {
            ...
            // Non-opaque so a background color with alpha < 1 (e.g. for window
            // blur) actually shows whatever is behind the layer instead of
            // being composited as if fully opaque regardless of alpha.
    ...
    public var nativeBackgroundColor: NSColor {
        ...
        set {
            ...
            // setupOptions() only paints this once at init; without refreshing it
            // here too, the view's own opaque CALayer backing stays stuck behind
            // the (possibly alpha < 1) color cells draw on top of, and the layer
            // background never becomes see-through no matter what alpha is set.
