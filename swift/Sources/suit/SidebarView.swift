...
// SSH Hosts / Notes / Bookmarks (in that order — see `railOrder`), picked from
...
final class SidebarView: NSView {
    ...
    enum Tab: Int, CaseIterable {
        ...
        // removed top tab strip), SSH, Notes, Bookmarks. Git is intentionally
        // absent — its changes/worktrees no longer get a dedicated icon; the
        // branch/worktree switcher lives on the Files footer, and the diff /
        // file-history / feedback / PR-inbox surfaces are reached on demand
        // through the palette (which still shows the GitView via showGit()).
        static let railOrder: [Tab] = [.files, .search, .sessions, .ssh, .notes, .bookmarks]
