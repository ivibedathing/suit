...
final class TerminalWindowController: NSObject, NSWindowDelegate, NSSplitViewDelegate, PaneHost {
    ...
    // the focused pane's project (the pre-Phase-9 behavior). Referenced by the
    // sidebar-pinning methods (TerminalWindowController+Sidebar.swift).
    ...
    // would close the wrong tab). Read/written by the tab-model helpers in
    // TerminalWindowController+TabModel.swift, so it must be internal.
